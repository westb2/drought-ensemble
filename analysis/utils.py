...
def load_run(data_accessor, outlet_yx, num_timesteps=8760, read_interval=4):
    ...
    # dz_vals = [dz_val*200.0 for dz_val in dz_vals]
    # dz_vals.reverse()
    ...
    run_data["water_required_to_recover"] = run_data["total_storage"][0]-run_data["total_storage"]# run_data["__precip"] = np.array(load_variable(lambda x: data_accessor.clm_forcing_apcp, time, data_accessor)*run_def["mask"][0] for time in run_data.time)
...
def plot_variable_for_each_run(variable_name, runs, title=None, x_scale=24.0*365.0,
    ...
    for run_name, run_data in runs.items():
        df[run_name] = run_data[variable_name][0:]/y_scale
    ...
    fig = px.line(melted_df, x='time', y=variable_name, color='Run',
                   title=title, template="simple_white", width=800, height=700).update_layout(
                    xaxis_title=x_label, yaxis_title=y_label, 
                    showlegend=True).update_layout(legend=dict(
                orientation="h",
                yanchor="bottom",
                y=.96,
                xanchor="right",
